...
void run_libdecode(double bonus_factor,
    ...
    if (err != cudaSuccess) {
        std::cerr << "Error in cudaGetDevice: " << cudaGetErrorString(err) << std::endl;
        exit(EXIT_FAILURE);
    }

    err = cudaGetDeviceProperties(&prop, device);
    if (err != cudaSuccess) {
        std::cerr << "Error in cudaGetDeviceProperties: " << cudaGetErrorString(err) << std::endl;
        exit(EXIT_FAILURE);
    }
    ...
    forwardKernel<<<1, NUM_CANDIDATES>>>(
        d_transition_likelihoods,
    ...
    h_ranking_offsets,
    num_candidates_per_function,
    cumulative_candidates_per_function,
    num_targets);*/
...
int main(int argc, char **argv) {
    ...
    std::string output_dir = parent_str + "RevDecode_results_naive_approach";
    ...
    for (const auto &input_file_path : input_files) {
        ...
        string output_file_path = output_dir + "/" + filename_stem + "_result_naive_approach.json";
